				...
				<div class="das2_2 pro6 ped14">
					...
					<div class="das2_3 das2_4">
						<div class="ped1">
							...
							<div class="ped2">
  <div class="ped6 ped3"> Nationality *: </div>
  <div class="ped6">
    <div class="ped7 ped8">
      <?php $nati = $doc_form->nationality ; ?>
      <select name="nationality" class="mnp_sel" required="required" id="natina"  onchange="selectNationality(this)">
        <?php  $nly = $wpdb->get_results("SELECT * FROM countries"); ?>
        <option value="">-- Select Type --</option>
        <?php foreach($nly as $country){ 
                  $n = $country->country; ?>
        <option value="<?php  echo $n; ?>" <?php if($nati == $n){echo "selected=selected";}?> id="natina_value">
        <?php  echo $n; ?>
        </option>
        <?php } ?>
      </select>
				...
				<div class="ped2">
                  ...
                  <?php  $ethni = $wpdb->get_results("SELECT * FROM ethnicity"); ?>
				<option value="">-- Select Type --</option>
				  <?php foreach($ethni as $country){ 
                  $et = $country->ethnicity; ?>
		...
		<input type="text" name="place_of_entry" placeholder="Place of Entry"  value="<?php echo $doc_form->place_of_entry; ?>"  required="required" id="plac_en"/>
							...
							<div class="ped2">
							 ...
							 <input class="datepicker" type="text" name="date_of_entry" placeholder="Date of Entry" required="required" value="<?php if(!empty($doc_form->date_of_entry)){echo $doc_form->date_of_entry;} ?>" id="dat_of_en">
									</div>
								</div>
							</div>
							<div class="ped2">
					...
					<input class="datepicker" type="text" name="date_of_visa_expiry" placeholder="Date of Visa Expiry" required="required" value="<?php if(!empty($doc_form->date_of_visa_expiry)){echo $doc_form->date_of_visa_expiry;} ?>" id="date_vi_ex"  >
									</div>
								</div>
							</div>
						</div>
	...
	<script>
//to enable,disable divs acording to selection
function selectNationality(element) {
    if (element.tagName.toLowerCase() === 'select')
        element = element.options[element.selectedIndex];
    
    var nat_val = element.value;
	
	if (nat_val == "United Kingdom" ) {
		$("#plac_en,#dat_of_en,#date_vi_ex").attr('disabled','disabled');
	}
	else
	{
		$("#plac_en,#dat_of_en,#date_vi_ex").attr('disabled',false);
	}
