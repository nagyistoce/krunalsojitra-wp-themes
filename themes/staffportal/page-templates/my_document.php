	...
	<div class="lol2">
					...
					<div class="myd3">
                       ...
                       <?php $sia_data = $wpdb->get_row("SELECT * FROM sia_licence WHERE sia_user_id = $user_id");?>
						  ...
						  $wpdb->query("UPDATE sia_licence SET sia_licence = '' WHERE sia_user_id = $user_id"); 
						  	$sia_lic_del_mes = "Your SIA Licence Deleted";
						  	wp_redirect( get_bloginfo('wpurl').'/my-document', 301 ); 
								  }
								  ?>
                                      <p style="color:#228BB9;"> <?php echo $sia_lic_del_mes;?></p>
									<div class="ped6 myd2">
		...
		//print_r($inst_email);
					...
					// echo "UPDATE sia_licence SET sia_licence = '$si_copy_licence' WHERE sia_user_id = $user_id";
	 				...
	 				$wpdb->query("INSERT INTO sia_licence VALUES ('', $si_user_id, '', '','',  '$si_copy_licence','')"); 
					//echo "INSERT INTO sia_licence VALUES ('', $si_user_id, '', '','',  '$si_copy_licence','')";
						}
					}
				wp_redirect( get_bloginfo('wpurl').'/my-document', 301 ); 
					 }
						...
						<div class="das2_3 das2_4">
							<div class="ped1">
								<div class="ped2">
									...
									<div class="ped6">
                                 			  <?php if($sia_data->driving_licence == ""){?>
										<img src="<?php bloginfo('stylesheet_directory'); ?>/images/no-image.jpg">                                        <?php }else{?>
									<img src="<?php bloginfo('stylesheet_directory'); ?>/upload/<?php echo $sia_data->driving_licence;?>" />						                                     <?php }?>
						  ...
						  $wpdb->query("UPDATE sia_licence SET driving_licence = '' WHERE sia_user_id = $user_id"); 
						  	$lic_del_mes = "Your Driving Licence Deleted";
								
								wp_redirect( get_bloginfo('wpurl').'/my-document', 301 );   }
								  ?>
                                 <p style="color:#228BB9;"> <?php echo $lic_del_mes;?></p>
									<div class="ped6 myd2">
									<form method="post" action="">
		...
		if (file_exists("wp-content/themes/staffportal/upload/" . uniqid(). $_FILES["file"]["name"]))
		  ...
		  //echo "Stored in: " . "upload/" . $_FILES["file"]["name"];
		   echo "<p  style='color:#2A97C7'>Driving Licence Upload</p>";
