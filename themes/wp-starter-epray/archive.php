					...
					<div class="grid">
						...
						<div class="grid__item desk--three-quarters home-post">
							 <div class="contant-home">
							 	 <?php if (is_category()) { ?>
								    <h6><?php _e("Posts Categorised:", "dmsqdtheme"); ?></span> <?php single_cat_title(); ?></h6>
							    	
							    <?php } elseif (is_tag()) { 
							    		$current_tag = single_tag_title("", false);
							    	?>
								    <h6><?php _e("Posts Tagged:", "dmsqdtheme"); ?></span> <?php single_tag_title(); ?></h6>
								    
							    <?php } elseif (is_author()) {
							    	global $post;
							    	$author_id = $post->post_author;
							    ?>
								    <h6><?php _e("Posts By:", "dmsqdtheme"); ?></span> <?php echo get_the_author_meta('display_name', $author_id); ?></h6>

								<?php } elseif (is_day()) { ?>
								    <h6><?php _e("Daily Archives:", "dmsqdtheme"); ?></span> <?php the_time('l, F j, Y'); ?></h6>
								
				    			<?php } elseif (is_month()) { ?>
					    		    <h6><?php _e("Monthly Archives:", "dmsqdtheme"); ?></span> <?php the_time('F Y'); ?></h6>
							    
							    <?php } elseif (is_year()) { ?>
							        <h6><?php _e("Yearly Archives:", "dmsqdtheme"); ?></span> <?php the_time('Y'); ?></h6>
							    <?php } ?>
                                      ...
                                      <?php echo "<p>".get_excerpt_with_hashtag(130)."</p>"; ?>
                                        ...
                                        <ul id="<?php echo get_the_ID(); ?>">
                                        	<?php
                                             ...
                                             <?php }else{?> 
													<a href="javascript:void(0);" class="like_plus">
			                                            <li class="like">
			                                                <span>
			                                                	<?php
			                                                	$postid = get_the_ID(); 
										    ...
										    <?php
                                               ...
                                               <li class="list">
	                                            ...
	                                            <?php }else{?> 
	                                            <a href="javascript:void(0);" class="prayer_list">
                                             	<li class="list">
	                                                <span><?php
			                                                	$postid = get_the_ID(); 
			                                                	 $prayerlistcount = $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->prefix."user_paryer_list WHERE post_id = $postid");
                                                ...
                                                <?php } ?>
                                            <a href="<?php the_permalink(); ?>#prayerform">
	                                            <li class="ref">
...
jQuery(document).ready(function() {
	...
	jQuery("#load_more_ajax a").click(function() {
		...
		data.authorid = <?php echo $authore_id; ?>;
		data.tag_name = "<?php echo $current_tag; ?>";
		data.action = "author_action";
		jQuery.post('<?php echo esc_url( home_url( '/' ) ); ?>wp-admin/admin-ajax.php',data, authorSuccess);
		
	});
	
	function authorSuccess(result) { // AJAX ONSCCESS FUNCTION
