...
$row = $wpdb->get_results("SELECT * from  notification");
...
if($pagination_count > 0) {
    ...
    //Get the data from the database
     $notification_data = $wpdb->get_results("SELECT * FROM notification ORDER BY notification_job_date DESC");
 echo "<br /><br /><div id='icon-edit-pages' class='icon32'></div><h1>Withdraw Event Notification</h1><br /><br />";
        ...
        <th>Event Timing</th>
            ...
            <tbody>
                ...
                foreach ($notification_data as $notification) {
						...
						$user_id =  $notification->notification_user_id;
						$post_id =	$notification->notification_post_id;
						$job_date = date('d-m-Y', strtotime($notification->notification_job_date));
						$job_event_name =	$notification->notification_job_event_name;
						$positions_name = $notification->notification_pos_name;
						$job_location =	$notification->notification_job_location;
						$job_time =	$notification->notification_job_time;
                    ?>
                <tr>
                    <td><?php echo $user_id; ?></td>
     <td><?php echo $job_event_name; ?></td>
     <td><?php echo $job_date; ?></td>
     <td><?php echo $positions_name; ?></td>
